...
public class DatabaseAuthenticationProviderTest extends DbUnitBaseTest {
  ...
  @Autowired private RemocraAuthenticationProvider underTest;
  ...
  public void testCreateUser() {
    ...
    Authentication auth =
        underTest.authenticate(
            new UsernamePasswordAuthenticationToken(user.getIdentifiant(), password));
  ...
  public void testAuthentication_DbUser() {
    ...
    Authentication auth =
        underTest.authenticate(new UsernamePasswordAuthenticationToken("maire", "admin"));
  ...
  public void testAuthentication_DbUser_EmailFail() {
    ...
    try {
      ...
      Authentication auth =
          underTest.authenticate(
              new UsernamePasswordAuthenticationToken("usertahtdoesnotexist", "admin"));
    } catch (Exception ex) {
      return;
    }
  ...
  public void testAuthentication_DbUser_PasswordFail() {
    ...
    try {
      ...
      Authentication auth =
          underTest.authenticate(
              new UsernamePasswordAuthenticationToken("maire", "adminwrongpassword"));
    } catch (Exception ex) {
  ...
  public void testAuthentication_DbUserHasProfile() throws BusinessException {
    ...
    when(profileProvider.getProfileAccessRights(
            any(ProfilUtilisateur.class), any(ProfilOrganisme.class)))
        .thenReturn(accessRights);
