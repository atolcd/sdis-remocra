...
//--
//-- Modification CVA. Gestion des dépendances
//--
Ext.require('Ext.ux.grid.MouseEventForwarding');
...
Ext.define('Ext.ux.grid.DataDrop', {
    ...
    dataDropped: function(){
        ...
        //--
        //-- Modification CVA. TODO Voir ces deux lignes en commentaires. Commentées -> collage ok, décommentées -> gissage ok depuis TXT (pas depuis Calc)
        //--
//        this.textEl.show();
//        this.textEl.blur();
        if (nv !== '') {
          nv = nv.replace(/\|/g,'');
          //--
          //-- Modification CVA. 1 : Evénement sur cmp. 2 : On utilise un Holder pour pour permettre la modification de la nv (remplacement de , par des points, etc.)
          //--
//        if (this.fireEvent('beforedatadrop',this,nv) !== false){
          var nvHolder = {'nv': nv};
          if (this.cmp.fireEvent('beforedatadrop',this,nvHolder) !== false){
        	nv = nvHolder.nv;
            ...
            Ext.each(columns, function(col){
                if (!col.hidden){
                	//--
                    //-- Modification CVA. On ne considère que les colonnes de données.
                    //--
                	if (col.isCheckerHd!==true) {
                		cols.push(col);
                	}
                }
            }, this);
            //--
            //-- Modification CVA. Evénement sur cmp.
            //--
            //if (cols.length && rows.length && this.fireEvent('datadrop',this,rows) !== false) {
            if (cols.length && rows.length && this.cmp.fireEvent('datadrop',this,rows) !== false) {
                for (var i = 0; i < rows.length; i++) {
                    var vals = rows[i].split(this.sepRe), data = {};
                    if (vals.length == 1){
                    	//--
                        //-- Modification CVA. Je n'ai pas touché le contenu de ce test : pourquoi ça a été écrit comme ça ?
                        //--
                        vals
                    }else{
                ...
                this.cmp.fireEvent('afterdatadrop',this,recs);
                this.resizeDropArea();
            }
