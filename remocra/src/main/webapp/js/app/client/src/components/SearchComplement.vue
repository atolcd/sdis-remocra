...
<form v-on:submit.prevent>
  <autocomplete :input-attrs="{placeholder: 'Sélectionner une valeur..' }" v-model="selected" :items="results" :get-label="getLabel" :component-item='complementTemplate' :min-len="2" :auto-select-one-item="true" @update-items="search" @item-selected="paramSelected"
...
export default {
  ...
  methods: {
    ...
    search(text) {
      axios.get('/remocra/evenements/evenementmodparalst/' + this.paramId + '?query=' + text + '&page=1&start=0&limit=10').then((response) => {
