  ...
  methods: {
          ...
          .then((response) => {
            ...
            if(evenement.criseComplement.length !== 0){
               _.forEach(evenement.criseComplement, complement => {
                 this.complements.push({id : complement.proprieteEvenement, valeurFormatee: complement.valeurFormatee, valeurSource :complement.valeurSource})
               })
            }
            this.loadComplement()
    ...
    loadComplement(){
      if(this.form.type !== null){
        axios.get('/remocra/evenements/proprietes/'+this.form.type)
          .then((response) => {
            if (response.data.data) {
