...
public class EmailModele implements ITypeReference {
  ...
  public static enum EmailModeleEnum {
    UTILISATEUR_MAIL_INSCRIPTION("UTILISATEUR_MAIL_INSCRIPTION"),
    UTILISATEUR_MAIL_MOT_DE_PASSE_PERDU("UTILISATEUR_MAIL_MOT_DE_PASSE_PERDU"),
    UTILISATEUR_MAIL_MOT_DE_PASSE("UTILISATEUR_MAIL_MOT_DE_PASSE"),
    CREATION_RCI("CREATION_RCI"),
    DEPOT_DELIB("DEPOT_DELIB"),
    DEPOT_DECLAHYDRANT("DEPOT_DECLAHYDRANT"),
    DEPOT_RECEPTRAVAUX("DEPOT_RECEPTRAVAUX");
  ...
  public static EmailModele findByValue(EmailModeleEnum value) {
    TypedQuery<EmailModele> findEmailModeleByCode =
        EmailModele.findEmailModelesByCode(value.getValue());
  ...
  public String replaceOccurences(EmailModeleKeyMap args, String stringToReplace) {
    if (stringToReplace != null && args != null && args.size() != 0) {
      for (EmailModeleKeys key : args.keySet()) {
        stringToReplace =
            stringToReplace.replace("[" + key + "]", StringUtils.stripToEmpty(args.get(key)));
      }
    }
