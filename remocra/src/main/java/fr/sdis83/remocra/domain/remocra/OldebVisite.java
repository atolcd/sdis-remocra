...
public class OldebVisite {
  ...
  @ManyToMany
  @JoinTable(
      name = "oldeb_visite_anomalie",
      joinColumns = {@JoinColumn(name = "visite", nullable = false)},
      inverseJoinColumns = {@JoinColumn(name = "anomalie", nullable = false)})
  ...
  @Formula(
      "(select tod.nom from remocra.type_oldeb_debroussaillement tod where tod.id = debroussaillement_acces)")
  private String nomDebAcces;

  @Formula(
      "(select tod.nom from remocra.type_oldeb_debroussaillement tod where tod.id = debroussaillement_parcelle)")
