...
@RooJavaBean
@RooToString
@RooJpaActiveRecord(versionField = "", finders = { "findOldebsByCommune" })
@RooJson
public class Oldeb implements Featurable {
    ...
    @ManyToMany
    @JoinTable(name = "oldeb_caracteristique", joinColumns = { @JoinColumn(name = "oldeb", nullable = false) }, inverseJoinColumns = {
    ...
    // Cas quand :
    @Formula(value = " CASE BTRIM(voie)"
            // voie est null
            + " WHEN null THEN null"
            // voie contient une chaine vide
            + " WHEN '' THEN null"
            // retour de la concaténation des valeurs
            + " ELSE BTRIM(COALESCE(num_voie, '') || ' ' || voie || ' ' || COALESCE(lieu_dit, '')) " + "END")
    private String adresse;

    // ov.date_visite desc limit 1 utilisé pour avoir la date dernière visite
    @Formula("(select ov.date_visite from remocra.oldeb_visite ov where ov.oldeb = id order by ov.date_visite desc limit 1)")
    ...
    @Formula(" (select tod.nom from remocra.oldeb_visite ov inner join remocra.type_oldeb_debroussaillement tod on (ov.debroussaillement_parcelle = tod.id)"
            + "where ov.oldeb = id order by ov.date_visite desc limit 1) ")
    private String debroussaillement;

    @Formula("(select toc.nom from remocra.oldeb_visite ov inner join remocra.type_oldeb_avis toc on (ov.avis = toc.id) "
            + "where ov.oldeb = id order by ov.date_visite desc limit 1)")
    private String avis;
