...
public class HydrantIndispoTemporaire {
  ...
  // @Transient
  // @Formula("(select count(*) from remocra.hydrant_indispo_temporaire_hydrant ith where
  // ith.indisponibilite = id)")

  @OneToMany
  @JoinTable(
      name = "hydrant_indispo_temporaire_hydrant",
      joinColumns = {@JoinColumn(name = "indisponibilite", nullable = false)},
      inverseJoinColumns = {@JoinColumn(name = "hydrant", nullable = false)})
