...
public class GeometryUtil {
  ...
  public static double[] transformCordinate(
    ...
    CoordinateOperation transformation =
        CoordinateOperationFactory.createCoordinateOperations(
                (GeodeticCRS) fromCoordRefSys, (GeodeticCRS) toCoordRefSys)
            .get(0);
    return transformation.transform(new double[] {longitude, latitude});
  }

  /**
   * Recherche des coordonnées DFCI. Réalisé en JDBC natif pour éviter une gestion de transactions
   * par Spring qui annule toutes les requêtes suivantes dans le cas où la table n'existe pas (cas
   * hors SDIS 83 par exemple)
   *
   * @param ds
   * @param geom
   * @return
  ...
  public static String findCoordDFCIFromGeom(DataSource ds, Geometry geom)
    ...
    logger.info(
        "Impossible de récupérer les coordonnées DFCI (table remocra_referentiel.carro_dfci)");
  ...
  public static Geometry getMultiGeometry(Geometry geometry) {
    ...
    if (geometry instanceof Point) {
      ...
      outputGeometry =
          geometryFactory.createMultiLineString(GeometryFactory.toLineStringArray(lineStringList));
