...
public class DepotController extends AbstractRemocraController {
    ...
    protected ResponseEntity<java.lang.String> uploadDepot(MultipartHttpServletRequest request, TypeDocument typeDocument) {
        try {
            ...
            return new SuccessErrorExtSerializer(true, "Le fichier" + (fichier == null ? "" : " '" + fichier + "'") + " a bien été déposé.").serialize();
    ...
    protected void saveDepotFile(MultipartFile mf, TypeDocument typeDocument) throws Exception {
        ...
        if (emailDestinataire == null) {
            ...
            logger.debug("Depôt " + typeDocument + " : modèle d'email non renseigné. Pas d'envoi d'email");
        } else {
            mailUtils.envoiEmailWithModele(
                    emailModele,
                    getSystemUtilisateur(),
                    emailDestinataire,
                    EmailModele.emptyKeyMap().add(EmailModeleKeys.URL_SITE, paramConfService.getUrlSite()).add(EmailModeleKeys.CODE, d.getCode())
