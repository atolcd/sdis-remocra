...
public class TelechargementController {
    ...
    public void downloadPdiDocument(@PathVariable("code") String code, HttpServletResponse response)
            throws IOException {
        DocumentUtil.getInstance().downloadDocument(telechargementsService.getPdiFilePathFromCode(code), code,
                response);
    }

    @RequestMapping(value = "/document/{code}")
    public void downloadRemocraDocument(@PathVariable("code") String code, HttpServletResponse response)
            throws IOException {
        DocumentUtil.getInstance().downloadDocument(telechargementsService.getRemocraFilePathFromCode(code), code,
                response);
    }
    ...
    public void showRemocraDocument(@PathVariable("code") String code, HttpServletResponse response)
            throws IOException {
        DocumentUtil.getInstance().showDocument(telechargementsService.getRemocraFilePathFromCode(code), code,
                response);
    }
