...
public class TraitementsController {
     ...
     * pour la thématique. Tout traitement hors spécifique (puis les traitements
     * dits spécifiques sont là pour outrepasser ça justement).
     * 
     * @param thematiqueId
     *            La thématique concernée par le test.
    ...
    protected void checkRightsForThematique(int thematiqueId) throws AccessDeniedException {
        if (authUtils.hasRight(TypeDroitEnum.REFERENTIELS, Permission.CREATE)) {
    ...
    /**
     * Vérifie que l'utilisateur a au moins un accès qui est censé le laisser
     * accéder à la brique des traitements. Tout traitement hors spécifique
     * (puis les traitements dits spécifiques sont là pour outrepasser ça
     * justement).
     * 
     * @throws AccessDeniedException
     *             Si l'utilisateur n'a pas les accès attendus.
     ...
     * @return
     */
    @RequestMapping(value = "modelestraitement/{id}", method = RequestMethod.GET, headers = "Accept=application/json")
    // @PreAuthorize : tests internes pour gérer les spécificités
    public ResponseEntity<java.lang.String> getModeles(final @PathVariable("id") Long id, @RequestParam(value = "filter", required = false) String filters) {
        ...
        final Long code = filtreCode != null && !filtreCode.getValue().equals("") ? Long.valueOf(filtreCode.getValue()) : null;

        return new AbstractExtListSerializer<ModeleTraitement>("Modeles Traitement retrieved.") {
            @Override
            protected JSONSerializer additionnalIncludeExclude(JSONSerializer serializer) {
                return serializer.include("data.idmodele").include("data.description").exclude("data.code").exclude("data.ref_chemin").exclude("data.ref_nom").exclude("data.type")
                        .exclude("data.version").exclude("data.message_echec").exclude("data.message_succes");
            }
     ...
     * @return
     */
    @RequestMapping(value = "modeletraitementparam/{id}", method = RequestMethod.GET, headers = "Accept=application/json")
    // @PreAuthorize : tests internes pour gérer les spécificités
    public ResponseEntity<java.lang.String> getParamModeles(final @PathVariable("id") Integer idModele) {

        return new AbstractExtListSerializer<ModeleTraitementParametre>("Paramètres Modeles Traitement retrieved.") {
            ...
            protected List<ModeleTraitementParametre> getRecords() {
                ...
                Query query = entityManager.createQuery("Select o from ModeleTraitementParametre o where o.idmodele = :idmodele  order by o.formNumOrdre");
    ...
    // @PreAuthorize : tests internes pour gérer les spécificités
    public ResponseEntity<java.lang.String> createTraitement(final @PathVariable("idmodele") Integer idmodele, final @RequestParam("jsonValeurs") String json,
     ...
     * @param json
     *            : paramètres du traitement associés aux valeurs saisies
     * @param request
     *            : Requête Http
     * @return
     * @throws BusinessException
     */
    public ResponseEntity<java.lang.String> doCreateTraitement(final @PathVariable("idmodele") Integer idmodele, final @RequestParam("jsonValeurs") String json,
            HttpServletRequest request) throws BusinessException {
    ...
    public ResponseEntity<java.lang.String> createTraitementSpecifiqueAtlas(HttpServletRequest request) throws BusinessException {
        ...
        tp.setIdparametre(ModeleTraitementParametre.findModeleTraitementParametresByNomAndIdmodele("LST_UTILISATEURS", modele).getSingleResult());
    ...
    @PreAuthorize("hasRight('REFERENTIELS', 'CREATE') or hasRight('HYDRANTS_EXPORT_NON_NUM', 'CREATE') or hasRight('HYDRANTS_TRAITEMENT', 'CREATE')")
    public ResponseEntity<java.lang.String> createTraitementSpecifiqueHydrantsNonNum(HttpServletRequest request) throws BusinessException {
        ...
        // ORGANISME_CIS_ID
        TraitementParametre tp = new TraitementParametre();
        tp.setIdparametre(ModeleTraitementParametre.findModeleTraitementParametresByNomAndIdmodele("ORGANISME_CIS_ID", modele).getSingleResult());
        tp.setIdtraitement(trt);
    ...
    /**
     * Sauvegarde en base une demande de traitement
     * "Nombre d'alertes par utilisateur"
     * 
     * @param request
     *            : Requête Http
     * @return
    ...
    @PreAuthorize("hasRight('REFERENTIELS', 'CREATE') or hasRight('ALERTES_EXPORT', 'CREATE')")
    public ResponseEntity<java.lang.String> createTraitementSpecifiqueAlertesUtilisateur(HttpServletRequest request) throws BusinessException {
        ...
        // DATE_DEB
        tp = new TraitementParametre();
        tp.setIdparametre(ModeleTraitementParametre.findModeleTraitementParametresByNomAndIdmodele("DATE_DEB", modele).getSingleResult());
        tp.setIdtraitement(trt);
        tp.setValeur("2013-01-01");
        paramArray[1] = tp;
        // DATE_FIN
        tp = new TraitementParametre();
        tp.setIdparametre(ModeleTraitementParametre.findModeleTraitementParametresByNomAndIdmodele("DATE_FIN", modele).getSingleResult());
        tp.setIdtraitement(trt);
