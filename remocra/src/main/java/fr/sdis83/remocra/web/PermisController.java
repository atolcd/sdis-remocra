...
public class PermisController extends AbstractRemocraController {
  ...
  @Autowired private PermisService permisService;
  ...
  protected static JSONSerializer additionnalIncludeExclude(JSONSerializer serializer) {
    return serializer
        ...
        .include("data.commune.pprif")
        .exclude("data.commune.*")
        // Documents
        .include("data.permisDocuments.id")
        .include("data.permisDocuments.titre")
        .include("data.permisDocuments.code")
        .exclude("data.permisDocuments.*");
  }

  @RequestMapping(value = "/searchxy", headers = "Accept=application/json")
  @PreAuthorize("hasRight('PERMIS_R')")
  public ResponseEntity<java.lang.String> searchPermis(
      final @RequestParam(value = "srid", required = true) Integer srid,
      final @RequestParam(value = "x", required = true) Float x,
      final @RequestParam(value = "y", required = true) Float y) {
  ...
  @PreAuthorize("hasRight('PERMIS_R')")
  public ResponseEntity<java.lang.String> searchPermis(
      final @RequestParam(value = "nom", required = false) String nom,
      final @RequestParam(value = "commune", required = false) Integer commune,
      ...
      final @RequestParam(value = "parcelleCadastrale", required = false) String parcelleCadastrale,
      final @RequestParam(value = "avis", required = false) Long avis) {
    return new AbstractExtListSerializer<Permis>("Permis retrieved.") {
      ...
      protected List<Permis> getRecords() {
        ...
        Geometry geom =
            utilisateurService
                ...
                .getZoneCompetence()
                .getGeometrie();
        return Permis.findPermisByNomCommEtc(
            srid,
            nom,
            commune,
            numero,
  ...
  @PreAuthorize("hasRight('PERMIS_C')")
  public ResponseEntity<java.lang.String> updatePermis(
      final @PathVariable("id") Long id,
      MultipartHttpServletRequest request,
      final @RequestParam String jsonPermis,
    ...
    try {
      ...
      if (authUtils.hasRight(TypeDroitEnum.PERMIS_DOCUMENTS_C)) {
        ...
        logger.error(
            "Dépôt de documents refusés ("
                + utilisateurService.getCurrentUtilisateur().getIdentifiant()
                + ")");
      }

      return new AbstractExtObjectSerializer<Permis>(
          "Permis created.", SuccessErrorExtSerializer.DEFAULT_CONTENT_TYPE) {
        @Override
        protected JSONSerializer additionnalIncludeExclude(JSONSerializer serializer) {
          return PermisController.additionnalIncludeExclude(serializer);
        }
  ...
  @RequestMapping(
      ...
      headers = "Content-Type=multipart/form-data")
  @PreAuthorize("hasRight('PERMIS_C')")
  public ResponseEntity<java.lang.String> createPermis(
      MultipartHttpServletRequest request,
      final @RequestParam String jsonPermis,
      final @RequestParam int fileCounter) {
    try {
