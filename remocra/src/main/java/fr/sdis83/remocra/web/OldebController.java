...
public class OldebController {
    ...
    @Autowired
    private OldebService oldebService;
    ...
    public ResponseEntity<java.lang.String> listJson(final @RequestParam(value = "page", required = false) Integer page,
        ...
        return new AbstractExtListSerializer<Oldeb>("Oldeb retrieved.") {
            ...
            protected JSONSerializer additionnalIncludeExclude(JSONSerializer serializer) {
                ...
                serializer.include("data.id").include("data.section").include("data.commune.id").include("data.parcelle").include("data.adresse").include("data.dateDerniereVisite")
                        .include("data.geometrie").include("data.nomZoneUrbanisme").include("data.debroussaillement").include("data.avis");

                return serializer.include("total").include("message").exclude("data.commune.geometrie").exclude("*");
            }
    ...
    public ResponseEntity<java.lang.String> getOldeb(final @RequestParam(value = "id", required = true) Long id) {
        ...
        return new AbstractExtObjectSerializer<Oldeb>("Oldeb retrieved.") {
            ...
            protected JSONSerializer additionnalIncludeExclude(JSONSerializer serializer) {
                // Retour des données de l'oldeb
                return serializer.include("data.id").include("data.section").include("data.parcelle").include("data.adresse").include("data.dateDerniereVisite")
                        .include("data.nomZoneUrbanisme").include("data.oldebVisites").include("data.oldebVisites.oldebVisiteSuites")
                        .include("data.oldebVisites.oldebVisiteDocuments").include("data.oldebVisites.typeOldebAnomalies").include("data.oldebVisites.nomAction")
                        .include("data.oldebVisites.nomDebAcces").include("data.oldebVisites.nomDebParcelle").include("data.typeOldebCaracteristiques")
                        .include("data.oldebProprietes.proprietaire")
                        /* .include("data.oldebProprietes.oldeb") */.include("data.oldebLocataires").include("data.debroussaillement").include("data.avis")
    ...
    public ResponseEntity<java.lang.String> updateOldeb(final @PathVariable Long id, MultipartHttpServletRequest request) {
        ...
        ArrayList<Integer> visiteDocumentsToDelete = visiteDocumentsToDeleteJson == null || visiteDocumentsToDeleteJson.isEmpty() ? new ArrayList<Integer>()
                : new JSONDeserializer<ArrayList<Integer>>().deserialize(visiteDocumentsToDeleteJson);
        Map<String, MultipartFile> files = request.getFileMap();
        try {
            final Oldeb attached = oldebService.update(id, json, visiteDocumentsToDelete.toArray(new Integer[] {}), files);
            if (attached != null) {
                return new AbstractExtObjectSerializer<Oldeb>("Oldeb updated.", SuccessErrorExtSerializer.DEFAULT_CONTENT_TYPE) {
                    @Override
        ...
        return new SuccessErrorExtSerializer(false, "Oldeb inexistant", HttpStatus.NOT_FOUND).serialize();
    ...
    public ResponseEntity<java.lang.String> createOldeb(MultipartHttpServletRequest request) {
        ...
        try {
            ...
            if (attached != null) {
                return new AbstractExtObjectSerializer<Oldeb>("Oldeb created.", SuccessErrorExtSerializer.DEFAULT_CONTENT_TYPE) {
                    @Override
                    protected Oldeb getRecord() throws BusinessException {
                        return attached;
                    }
    ...
    public ResponseEntity<java.lang.String> delete(@PathVariable("id") Long id) {
        try {
            ...
            return new SuccessErrorExtSerializer(true, "Obligation de débroussaillement supprimé").serialize();
        } catch (Exception e) {
            return new SuccessErrorExtSerializer(false, e.getMessage()).serialize();
        }
    }
    ...
    /**
     * Service de mise à jour de la géométrie de l'oldeb dont l'id est passé en
     * paramètre
     *
     * @param id,
     *            identifiant de l'obligation à mettre à jour
     * @param geometrie,
     *            nouvelle geometrie
     * @param srid,
     *            projection de la nouvelle geometrie
     * @return
     */
    ...
    public ResponseEntity<java.lang.String> updateGeom(final @PathVariable(value = "id") Long id, final @RequestParam(value = "geometrie") String wkt,
        ...
        try {
            ...
            return new SuccessErrorExtSerializer(false, "Problème survenu lors de la modification de la géométrie de l'obligation de débroussaillement").serialize();
        }

        return new SuccessErrorExtSerializer(true, "Géométrie de l'obligation de débroussaillement mise à jour.").serialize();
    }
