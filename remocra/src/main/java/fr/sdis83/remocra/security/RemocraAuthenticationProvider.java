...
/** Permet d'utiliser la base de données pour la gestion des utilisateurs de l'application. */
public class RemocraAuthenticationProvider extends AbstractUserDetailsAuthenticationProvider {
  ...
  @Autowired MessageDigestPasswordEncoder messageDigestPasswordEncoder;
  ...
  protected UserDetails retrieveUser(
    ...
    try {
      ...
      logger.debug(
          "Found user with identifiant : " + username + " with id : " + targetUtilisateur.getId());
      ...
      logger.debug(
          "Found organisme de type : "
              + organisme.getTypeOrganisme().getCode()
              + " pour l'utilisateur "
              + targetUtilisateur.getEmail());
      ...
      if (profilUtilisateur != null && organisme.getProfilOrganisme() != null) {
        try {
          authorities.addAll(
              provider.getProfileAccessRights(profilUtilisateur, organisme.getProfilOrganisme()));
          ...
          throw new AuthenticationServiceException(
              "Droit de profils incorrects. Configuration de l'utilisateur incorrecte. Contactez votre administrateur");
        }
      } else {
        throw new AuthenticationServiceException(
            "Profil non renseigné. Configuration de l'utilisateur incorrecte. Contactez votre administrateur");
      }
  ...
  /**
   * Dans tous les cas, on récupère l'utilisateur de la base en amont. On essaie de l'identifier à
   * partir de l'annuaire ou de Remocra.
   *
   * @param dbRetrievedUser
   * @param password
   * @throws BadCredentialsException
   */
  void validateCredentials(Utilisateur dbRetrievedUser, String password)
      throws BadCredentialsException {
    ...
    if (!StringUtils.hasText(expectedPassEncoded)) {
      throw new BadCredentialsException(
          "No password for "
              + dbRetrievedUser.getIdentifiant()
              + " set in database, contact administrator");
    }

    if (!messageDigestPasswordEncoder
        .encodePassword(password, dbRetrievedUser.getSalt())
        .equals(expectedPassEncoded)) {
