...
public class BlocDocumentService extends AbstractService<BlocDocument> {
  ...
  @Transactional
  @Override
  public BlocDocument setUpInformation(
      BlocDocument attached, Map<String, MultipartFile> files, Object... params) throws Exception {

    MultipartFile file =
        files != null && !files.isEmpty() ? files.values().iterator().next() : null;
