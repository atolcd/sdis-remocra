...
public class VoieService {
    ...
    public List<Voie> findVoiesByMotClassantOrNomLike(int firstResult, int maxResults, List<ItemSorting> itemSortings, List<ItemFilter> itemFilters) {
        ...
        if (wktFilter != null) {
            ...
            orders.add(cBuilder.asc(cBuilder.function("st_distance", Double.class, geometrie, wktOrderParam)));
    ...
    public Predicate[] makeFilterPredicates(Map<String, Object> parameters, Root<Voie> from, List<ItemFilter> itemFilters) {
    ...
    private Predicate mcOrNomFieldPredicate(Root<Voie> from, ItemFilter itemFilter) {
        ...
        Predicate motClassantP = cBuilder.like(cBuilder.lower(motClassant), "%" + nom.toLowerCase() + "%");
    ...
    private Predicate wktFieldPredicate(Map<String, Object> parameters, Root<Voie> from, ItemFilter itemFilter) {
        CriteriaBuilder cBuilder = Voie.entityManager().getCriteriaBuilder();
        ...
        Predicate within = cBuilder.isTrue(cBuilder.function("st_dwithin", Boolean.class, geometrie, wktParam, distParam));
