...
public class DebitSimultaneService extends AbstractService<DebitSimultane> {
  ...
  public List<DebitSimultane> getDebitSimultaneFromLonLat(
      Double lon, Double lat, Integer srid, Integer distance) {
    TypedQuery<DebitSimultane> query =
        entityManager
            .createQuery(
                "SELECT o FROM DebitSimultane o where st_distance(geometrie, ST_Transform(ST_SetSRID(ST_makePoint(:lon, :lat), :srid), :geomsrid)) < :distance",
  ...
  public String checkDs(String hydrants) {
    ...
    for (Integer id : ids) {
      List<String> numeros =
          entityManager
              .createNativeQuery(
                  "select h.numero from remocra.hydrant h join remocra.debit_simultane_hydrant"
                      + " dsh on dsh.hydrant = h.id where dsh.hydrant=:id")
