...
public class InterventionRepository {
  ...
  public List<Intervention> getByCrise(Long id) {
    ...
    l = context.select().from(CRISE_INTERVENTION).
        join(INTERVENTION)
  .on(CRISE_INTERVENTION.INTERVENTION.eq(INTERVENTION.ID))
        .where(CRISE_INTERVENTION.CRISE.eq(id)).fetchInto(Intervention.class);
    return l;
  }
  ...
  public String getXmlIndicateur(String code){
    try {
     CriseIndicateur cri = context.select().from(CRISE_INDICATEUR).where(CRISE_INDICATEUR.CODE.eq(code)).fetchInto(CriseIndicateur.class).get(0);
 String s = cri.getSourceSql();
      Object obj = entityManager.createNativeQuery(String.valueOf(s)).getSingleResult();
      return obj.toString();
    } catch(Exception e){
      logger.error("Une erreur est survenue lors de la tentative d'exécution de la requête "+code);
