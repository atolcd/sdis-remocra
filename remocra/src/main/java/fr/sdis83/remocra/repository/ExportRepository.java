...
public class ExportRepository {
  ...
  public String getExportFilePathFromCode(String code) {
    try {
      ProcessusEtlDocument ped =
          context
              .select()
              .from(PROCESSUS_ETL_DOCUMENT)
              .where(PROCESSUS_ETL_DOCUMENT.CODE.eq(code))
              .fetchInto(ProcessusEtlDocument.class)
              .get(0);
      if (ped == null) {
        return null;
      }
      Document d =
          context
              .select()
              .from(DOCUMENT)
              .join(PROCESSUS_ETL_DOCUMENT)
              .on(DOCUMENT.ID.eq(PROCESSUS_ETL_DOCUMENT.DOCUMENT))
              .where(PROCESSUS_ETL_DOCUMENT.CODE.eq(code))
  ...
  public boolean setExportAccuseFromCode(String code) {
    try {
      ...
      if (ped == null) {
        ...
        context
            .update(PROCESSUS_ETL_DOCUMENT)
            .set(PROCESSUS_ETL_DOCUMENT.ACCUSE, new Instant())
            .where(PROCESSUS_ETL_DOCUMENT.ID.eq(ped.getId()))
            .execute();
      }
      return true;
    } catch (Exception e) {
