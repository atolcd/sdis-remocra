...
public class RepertoireLieuRepository {
  ...
  private List<RepertoireLieuData> getRepertoireLieuData(
      String query, List<RepertoireLieu> repertoires) {
    ...
    String queryReplace =
        query
            ...
            // "aire gen voyag gorr" pour trouver "aire d'accueil des gens du voyage - GORRON
            // (Lieu-dit)"
            .replace(' ', '%');
    ...
    for (int i = 0; i < repertoires.size(); i++) {
      ...
      sql.append(" SELECT ")
          ...
          .append(" FROM(")
          .append(repertoireLieu.getSourceSql())
          .append(") AS rep")
          .append(repertoireLieu.getId())
          .append(" WHERE ")
          .append(repertoireLieu.getSourceSqlLibelle())
          .append(" ILIKE '%")
          .append(queryReplace)
  ...
  public List<RepertoireLieuData> getAllById(Long id, String query) {
    ...
    List<RepertoireLieu> repertoires =
        context
            .selectFrom(REPERTOIRE_LIEU)
            .where(
                REPERTOIRE_LIEU.ID.in(
                    DSL.select(CRISE_REPERTOIRE_LIEU.REPERTOIRE_LIEU)
