...
public class HydrantIndispoTemporaireRepository {
  ...
  public List<HydrantIndispoTemporaireRecord> getAll(List<ItemFilter> itemFilters, Integer limit, Integer start, List<ItemSorting> itemSortings) {
    ...
    List<HydrantIndispoTemporaireRecord> indispoTempRecordList = new ArrayList<HydrantIndispoTemporaireRecord>();
    StringBuffer sbReq = new StringBuffer("SELECT hit.id, hit.date_debut as dateDebut, hit.date_fin as dateFin, hit.motif,")
      ...
      .append("(SELECT c.nom FROM remocra.hydrant_indispo_temporaire hit2")
        ...
        .append(" JOIN remocra.commune c on c.id = h2.commune WHERE hit2.id = hit.id limit 1) as commune")
    ...
    for (Record r : hydrantIndispoTemporaireRecord) {
      HydrantIndispoTemporaireRecord indispoTemp = modelMapper.map(r, HydrantIndispoTemporaireRecord.class);
      Long idIndispoTemp = Long.valueOf(r.getValue("id").toString());

      // Statut
      TypeHydrantIndispoStatutRecord statut = context.selectFrom(TYPE_HYDRANT_INDISPO_STATUT)
          .where(TYPE_HYDRANT_INDISPO_STATUT.ID.eq(Long.valueOf(r.getValue("statut").toString())))
          .fetchOneInto(TypeHydrantIndispoStatutRecord.class);
      indispoTemp.setStatut(statut);

      // Hydrants
      StringBuffer reqHydrants = new StringBuffer("SELECT h.id, h.numero, ST_asGeoJSON(h.geometrie) as jsonGeometrie, ST_AsText(h.geometrie) as wktGeometrie")
        .append(" FROM remocra.hydrant h")
        .append(" JOIN remocra.hydrant_indispo_temporaire_hydrant hits on hits.hydrant = h.id")
        .append(" JOIN remocra.hydrant_indispo_temporaire hit on hit.id = hits.indisponibilite")
      ...
      for(Record h : context.fetch(reqHydrants.toString())) {
        ...
        CommuneRecord c = context.select(COMMUNE.ID, COMMUNE.NOM)
            .from(COMMUNE)
            .join(HYDRANT).on(HYDRANT.COMMUNE.eq(COMMUNE.ID))
  ...
  private String getFilters(List<ItemFilter> itemFilters) {
    ...
    for (ItemFilter itemFilter : itemFilters) {
      if("statut".equals(itemFilter.getFieldName())) {
        ...
        StringBuffer communeReq = new StringBuffer(" AND "+itemFilter.getValue()+" IN (SELECT c.id FROM remocra.hydrant_indispo_temporaire hit2")
          .append(" JOIN remocra.hydrant_indispo_temporaire_hydrant hith2 ON hith2.indisponibilite = hit2.id")
          .append(" JOIN remocra.hydrant h2 on h2.id = hith2.hydrant")
          .append(" JOIN remocra.commune c on c.id = h2.commune WHERE hit2.id = hit.id LIMIT 1)");
        condition += communeReq.toString();
      } else if("hydrantId".equals(itemFilter.getFieldName())) {
        StringBuffer hydrantReq = new StringBuffer(" AND "+itemFilter.getValue()+" IN (SELECT hydrant")
  ...
  /**
   * Calcule la géométrie d'une indisponibilité temporaire
   * La géométrie de l'IT est l'enveloppe de la géométrie des points d'eau composant l'IT
   */
