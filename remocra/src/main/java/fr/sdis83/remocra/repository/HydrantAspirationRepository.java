...
public class HydrantAspirationRepository {
   ...
   */
  public void addHydrantAspirationFromFiche(Long id, String data) throws IOException, CRSException, IllegalCoordinateException {

    if(data != null && !"null".equals(data)) {
      List<Map<String, Object>> listeAspirations = objectMapper.readValue(data, new TypeReference<List<Map<String, Object>>>(){});
  ...
  public void deleteHydrantAspirationsFromFiche(Long id, String data) throws IOException {
    if(data != null && !"null".equals(data)) {
      ArrayList<Long> listeAspi = objectMapper.readValue(data.toString(), new TypeReference<ArrayList<Long>>(){});
  ...
  /**
   * Ajoute une aspiration en base
   * La fonction updatehydrantAspiration est ensuite appelée pour lui transmettre les données
  ...
  private HydrantAspiration createHydrantAspiration(Long idPena, HydrantAspiration aspiration) {
    Long id = context.insertInto(HYDRANT_ASPIRATION)
      .set(HYDRANT_ASPIRATION.PENA, idPena)
      .returning(HYDRANT_ASPIRATION.ID).fetchOne().getValue(HYDRANT_ASPIRATION.ID);
    aspiration.setId(id);
  ...
  private HydrantAspiration updateHydrantAspiration(HydrantAspiration aspiration) {
    context.update(HYDRANT_ASPIRATION)
    ...
    return context.selectFrom(HYDRANT_ASPIRATION)
      .where(HYDRANT_ASPIRATION.ID.eq(aspiration.getId()))
  ...
  private void deleteHydrantAspiration(Long id) {
    context.deleteFrom(HYDRANT_ASPIRATION)
      .where(HYDRANT_ASPIRATION.ID.eq(id))
      .execute();
  }
}
