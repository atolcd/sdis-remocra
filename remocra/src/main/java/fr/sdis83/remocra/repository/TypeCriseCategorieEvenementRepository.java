...
public class TypeCriseCategorieEvenementRepository {
  ...
  @Bean
  public TypeCriseCategorieEvenementRepository typeCriseCategorieEvenementRepository(
      DSLContext context) {
  ...
  public List<TypeCriseCategorieEvenement> getAll(List<ItemFilter> itemFilters, Long id)
      throws BusinessException {

    // On filtre par type_crise et par profil_droit
    Long tc =
        context.select(CRISE.TYPE_CRISE).from(CRISE).where(CRISE.ID.eq(id)).fetchOne().value1();
    List<TypeCriseCategorieEvenement> l = null;
    l =
        context
            .select()
            .from(TYPE_CRISE_CATEGORIE_EVENEMENT)
            .join(TYPE_CRISE_EVENEMENT_CRISE)
            .on(
                TYPE_CRISE_EVENEMENT_CRISE.CATEGORIE_EVENEMENT.eq(
                    TYPE_CRISE_CATEGORIE_EVENEMENT.ID))
            .join(TYPE_CRISE_EVENEMENT_DROIT)
            .on(
                TYPE_CRISE_EVENEMENT_DROIT.CATEGORIE_EVENEMENT.eq(
                    TYPE_CRISE_CATEGORIE_EVENEMENT.ID))
            .where(
                TYPE_CRISE_EVENEMENT_DROIT.PROFIL_DROIT.eq(
                    utilisateurService.getCurrentProfilDroit().getId()))
            .and(TYPE_CRISE_EVENEMENT_CRISE.TYPE_CRISE.eq(tc))
            .fetchInto(TypeCriseCategorieEvenement.class);
    return l;
  }
}
