...
public class ContactRolesRepository {
  ...
  ContactRolesRepository(DSLContext context) {
  ...
  public void deleteContactRolesById(Long idContact) {
    context.deleteFrom(CONTACT_ROLES).where(CONTACT_ROLES.CONTACT.eq(idContact)).execute();
  }

  public void createContactRoles(Long idContact, Long idRole) {
    context
        .insertInto(CONTACT_ROLES, CONTACT_ROLES.CONTACT, CONTACT_ROLES.ROLES)
