...
public class DestinataireRepository {
  ...
  public List<Integer> getAllIdOrganismes() {
    Result<Record1<Integer>> result =
        context.select(ORGANISME.ID.cast(SQLDataType.INTEGER)).from(ORGANISME).fetch();
   ...
   * @param strict Boolean si vrai : recherche strict, sinon : recherche la dans les résultats
   *     potentiels
   * @return liste de destinataires provenant de la table UTILISATEUR
   */
  public List<DestinataireModel> getDestinataireUtilisateur(
      List<Integer> idOrganismes, String filtre, Boolean strict) {
    List<DestinataireModel> destinataires =
        context
            .select(
                UTILISATEUR.ID,
            ...
            .where(
                UTILISATEUR
                    ...
                    .and(
                        (lower(UTILISATEUR.NOM).like(setFiltre(filtre, strict)))
    ...
    List<DestinataireModel> destinataires =
        context
            .select(
                ...
                DSL.val("Organisme").as("type"))
            .from(ORGANISME)
            .where(ORGANISME.ID.in(idOrganismes))
            .and(
                ORGANISME
                    .NOM
                    .likeIgnoreCase(setFiltre(filtre, strict))
    ...
    List<DestinataireModel> destinataires =
        context
            .select(
                ...
                CONTACT.EMAIL,
                CONTACT.FONCTION.as("function"),
                DSL.val("Contact").as("type"))
            .from(CONTACT)
        ...
        context
            ...
            .where(
                CONTACT
                    ...
                    .and(
                        (lower(CONTACT.NOM).like(setFiltre(filtre, strict)))
