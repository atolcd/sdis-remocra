...
public class AccesAPIOrganismeRepository {
    ...
    public void newDdeApi(Long idOrganisme){
        ...
        String code = messageDigestPasswordEncoder.encodePassword(new Date().getTime() + String.valueOf(idOrganisme), null);
        context.insertInto(DDE_API, DDE_API.ORGANISME, DDE_API.CODE, DDE_API.DATE_DEMANDE)
                .values(idOrganisme, code, new Instant()).execute();
    ...
    public void insertMailClefAPI(Long idOrganisme, String code){
        ...
        String corpsMail = context.select(EMAIL_MODELE.CORPS).from(EMAIL_MODELE).where(EMAIL_MODELE.CODE.eq("ACCES_API")).fetchOne(EMAIL_MODELE.CORPS);
        String objetMail = context.select(EMAIL_MODELE.OBJET).from(EMAIL_MODELE).where(EMAIL_MODELE.CODE.eq("ACCES_API")).fetchOne(EMAIL_MODELE.OBJET);

        CharSequence key = "[URL_API]";
        CharSequence value = context.select(PARAM_CONF.VALEUR).from(PARAM_CONF).where(PARAM_CONF.CLE.eq("PDI_URL_SITE")).fetchOne(PARAM_CONF.VALEUR)+"#organismesAPI/index/code/"+code;
        corpsMail = corpsMail.replace(key, value);

        String nomDestinataire = context.select(ORGANISME.NOM).from(ORGANISME).where(ORGANISME.ID.eq(idOrganisme)).fetchOne(ORGANISME.NOM);
        String emailDestinataire = context.select(ORGANISME.EMAIL_CONTACT).from(ORGANISME).where(ORGANISME.ID.eq(idOrganisme)).fetchOne(ORGANISME.EMAIL_CONTACT);;
        ...
        context.insertInto(EMAIL, EMAIL.CORPS, EMAIL.DESTINATAIRE,EMAIL.DESTINATAIRE_EMAIL, EMAIL.EXPEDITEUR, EMAIL.EXPEDITEUR_EMAIL, EMAIL.OBJET)
                .values(corpsMail, nomDestinataire, emailDestinataire, nomExpediteur, mailExpediteur, objetMail).execute();
    }

    public boolean getCodeValidity(String code){
        //On regarde si le code du lien est toujours valide avec la date
        String dateDemandeString = context.select(DDE_API.DATE_DEMANDE).from(DDE_API).where(DDE_API.CODE.eq(code)).fetchOneInto(Instant.class).toString();
        ...
        boolean used = context.select(DDE_API.UTILISE).from(DDE_API).where(DDE_API.CODE.eq(code)).fetchOneInto(Boolean.class);
    ...
    public void setPasswordOrganisme(String code, String password) throws Exception {
        if (this.checkPasswordValidity(password)){
            ...
            Long idOrganisme = context.select(DDE_API.ORGANISME).from(DDE_API).where(DDE_API.CODE.eq(code)).fetchOneInto(Long.class);

            context.update(ORGANISME)
                    .set(ORGANISME.PASSWORD, pwdSalt)
                    .set(ORGANISME.SALT, salt)
                    .where(ORGANISME.ID.eq(idOrganisme)).execute();
