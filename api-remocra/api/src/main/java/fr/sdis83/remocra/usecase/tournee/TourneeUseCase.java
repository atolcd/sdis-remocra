...
public class TourneeUseCase {
    ...
    public ReservationTourneesResponse reserveTournees(List<Long> listIdTournees, Long idUser) {
        ...
        List<TourneeModel> dejaReservees = tournees.stream().filter(t -> t.reservation() != null).collect(Collectors.toList());

        // On supprime les tournées déjà réservées
        tournees.removeAll(dejaReservees);

        // Puis, on réserve celle qu'on peut
        int result = tourneeRepository.reserveTournees(
                tournees.stream().map(TourneeModel::idRemocra).
        ...
        Map<Long, List<Long>> mapHydrantsByTournee = tourneeRepository.getHydrantsByTournee(tournees.stream().map(TourneeModel::idRemocra).
                collect(Collectors.toList()));
        ArrayList<TourneeModel> listeTourneeAvecHydrant = new ArrayList<>();
        tournees.stream().forEach(tournee ->
                listeTourneeAvecHydrant.add(ImmutableTourneeModel.builder()
                        .idRemocra(tournee.idRemocra())
                        .nom(tournee.nom())
                        .affectation(tournee.affectation())
                        .reservation(tournee.affectation())
                        .listeHydrant(mapHydrantsByTournee.get(tournee.idRemocra())).build())
        );

        // On retourne les tournées réservées et celles qu'on n'a pas pu réserver
        return new ReservationTourneesResponse(
                listeTourneeAvecHydrant, dejaReservees);
    }
    ...
    static class ReservationTourneesResponse {
        ...
        public ReservationTourneesResponse( List<TourneeModel> tourneesReservees, List<TourneeModel> tourneesNonReservees) {
