...
public class PeiRepository {
  ...
  public boolean peiExist(String numero) {
    return context.fetchExists(
        context.select(HYDRANT.ID).from(HYDRANT).where(HYDRANT.NUMERO.equalIgnoreCase(numero)));
  }
  ...
  public List<PeiModel> getAll(
      ...
      Integer start,
      Long userId,
      String userTypeOrganisme) {
    Condition condition =
        this.getConditions(insee, type, codeNature, natureDeci, userId, userTypeOrganisme);
    ...
    Field<Object> LONG =
        DSL.field("round(st_x(st_transform(hydrant.geometrie, 4326))::numeric, 8)").as("lon");
    Field<Object> LAT =
        DSL.field("round(st_y(st_transform(hydrant.geometrie, 4326))::numeric, 8)").as("lat");
    ...
    return context
        .select(
            ...
            DSL.when(
                    HYDRANT
                        .DISPO_TERRESTRE
                        .eq("DISPO")
                        .or(HYDRANT.DISPO_TERRESTRE.eq("NON_CONFORME")),
                    true)
                .otherwise(false)
                .as("disponible"),
        ...
        .leftJoin(TYPE_HYDRANT_NATURE)
        .on(TYPE_HYDRANT_NATURE.ID.eq(HYDRANT.NATURE))
        .leftJoin(HYDRANT_PIBI)
        .on(HYDRANT_PIBI.ID.eq(HYDRANT.ID))
        .leftJoin(TYPE_HYDRANT_DIAMETRE)
        .on(TYPE_HYDRANT_DIAMETRE.ID.eq(HYDRANT_PIBI.DIAMETRE))
        .leftJoin(TYPE_HYDRANT_DOMAINE)
        .on(TYPE_HYDRANT_DOMAINE.ID.eq(HYDRANT.DOMAINE))
        .leftJoin(HYDRANT_PENA)
  ...
  public PeiSpecifiqueModel getPeiSpecifique(String numero) {
        ...
        context
            .select(
                ...
                DSL.greatest(
                        ...
                        HYDRANT.DATE_RECO,
                        HYDRANT.DATE_CREA)
                    .as("dateDerniereVisite"),
                HYDRANT.DISPO_TERRESTRE.as("dispoTerrestre"),
                HYDRANT.DISPO_HBE.as("dispoAerienne"))
            .from(HYDRANT)
            .leftJoin(TYPE_HYDRANT_NATURE)
            .on(TYPE_HYDRANT_NATURE.ID.eq(HYDRANT.NATURE))
            .leftJoin(autoritePolice)
            .on(autoritePolice.ID.eq(HYDRANT.ORGANISME))
            .leftJoin(servicePublicDeci)
            .on(servicePublicDeci.ID.eq(HYDRANT.AUTORITE_DECI))
            .leftJoin(TYPE_HYDRANT_NATURE_DECI)
            .on(TYPE_HYDRANT_NATURE_DECI.ID.eq(HYDRANT.NATURE_DECI))
            .leftJoin(COMMUNE)
            .on(COMMUNE.ID.eq(HYDRANT.COMMUNE))
            .leftJoin(TYPE_HYDRANT_NIVEAU)
            .on(TYPE_HYDRANT_NIVEAU.ID.eq(HYDRANT.NIVEAU))
            .leftJoin(TYPE_HYDRANT_DOMAINE)
            .on(TYPE_HYDRANT_DOMAINE.ID.eq(HYDRANT.DOMAINE))
            .leftJoin(HYDRANT_PIBI)
            .on(HYDRANT_PIBI.ID.eq(HYDRANT.ID))
    ...
    Integer nbIndispoTemp =
        context
            ...
            .join(HYDRANT)
            .on(HYDRANT.ID.eq(HYDRANT_ANOMALIES.HYDRANT))
            .where(
                HYDRANT
                    .NUMERO
                    .eq(pei.getNumero())
                    .and(TYPE_HYDRANT_ANOMALIE.CODE.equalIgnoreCase("INDISPONIBILITE_TEMP")))
  ...
  public String getPeiCaracteristiques(String numero) throws JsonProcessingException {
    ...
    if (TypeHydrant.PIBI.getCode().equalsIgnoreCase(typePei)) {
  ...
  public String getPibiCaracteristiques(String numero) throws JsonProcessingException {
    ...
    fr.sdis83.remocra.db.model.remocra.tables.Hydrant pibiJumele = HYDRANT.as("pibiJumele");
    PibiModel pibi =
        context
            .select(
            ...
            .leftJoin(pibiJumele)
            .on(pibiJumele.ID.eq(HYDRANT_PIBI.JUMELE))
            .leftJoin(TYPE_HYDRANT_DIAMETRE)
            .on(TYPE_HYDRANT_DIAMETRE.ID.eq(HYDRANT_PIBI.DIAMETRE))
            .leftJoin(TYPE_HYDRANT_MARQUE)
            .on(TYPE_HYDRANT_MARQUE.ID.eq(HYDRANT_PIBI.MARQUE))
            .leftJoin(TYPE_HYDRANT_MODELE)
            .on(TYPE_HYDRANT_MODELE.ID.eq(HYDRANT_PIBI.MODELE))
            .leftJoin(TYPE_RESEAU_ALIMENTATION)
            .on(TYPE_RESEAU_ALIMENTATION.ID.eq(HYDRANT_PIBI.TYPE_RESEAU_ALIMENTATION))
            .leftJoin(TYPE_RESEAU_CANALISATION)
            .on(TYPE_RESEAU_CANALISATION.ID.eq(HYDRANT_PIBI.TYPE_RESEAU_CANALISATION))
            .where(HYDRANT.NUMERO.eq(numero))
  ...
  public String getPenaCaracteristiques(String numero) throws JsonProcessingException {
    PenaModel pena =
        context
            ...
            .leftJoin(TYPE_HYDRANT_MATERIAU)
            .on(TYPE_HYDRANT_MATERIAU.ID.eq(HYDRANT_PENA.MATERIAU))
            .where(HYDRANT.NUMERO.eq(numero))
  ...
  public void updatePibiCaracteristiques(HydrantPibi hydrantPibi, Integer anneeFabrication)
  ...
  */
  public String jumelagePei(String numero, String numeroJumeau) {
    ...
    if (idPeiJumeau != null) {
      fr.sdis83.remocra.db.model.remocra.tables.Hydrant h1 = HYDRANT.as("h1");
      fr.sdis83.remocra.db.model.remocra.tables.Hydrant h2 = HYDRANT.as("h2");
  ...
  */
  public boolean peiBi(String numero) {
    return context
        .select(DSL.field(TYPE_HYDRANT_NATURE.CODE.eq("BI")))
  ...
  */
  public void defaitJumelage(Long idPei) {
    ...
    if (idAncienJumeau != null) {
      context
          ...
          .where(HYDRANT_PIBI.ID.eq(idAncienJumeau))
          .execute();
  ...
  public int insertPena(Long idHydrantPena) {
    return context.insertInto(HYDRANT_PENA).set(HYDRANT_PENA.ID, idHydrantPena).execute();
  }

  public int insertPibi(Long idHydrantPibi) {
    return context.insertInto(HYDRANT_PIBI).set(HYDRANT_PIBI.ID, idHydrantPibi).execute();
  }

  public void deleteHydrant(Long idHydrant) {
    context.deleteFrom(HYDRANT).where(HYDRANT.ID.eq(idHydrant)).execute();
  }

  public Hydrant getByNumero(String numero) {
    return context
        .select()
        .from(HYDRANT)
        .where(HYDRANT.NUMERO.equalIgnoreCase(numero))
  ...
  public Hydrant getById(Long id) {
    return context.select().from(HYDRANT).where(HYDRANT.ID.eq(id)).fetchOneInto(Hydrant.class);
  }

  /**
   * Retourne les identifiants de communes associés à la liste des numéros de PEI passée en
   * paramètre
   *
  ...
  public List<Long> getDistinctIdsCommuneFromListPei(List<String> listNumeroPei) {
    return context
        .selectDistinct(HYDRANT.COMMUNE)
        .from(HYDRANT)
