...
public static HttpServerWebappModule create(Config config) {
        return new HttpServerWebappModule(ImmutableHttpServerWebappModule.Settings.builder()
                                          .port(config.getInt("port")).gracefulStopTime(config.getInt("graceful-stop-time"))
                                          .docPath(config.getString("doc-path"))
                                          .documentationAuthorizedIp(new HashSet<String>(config.getStringList("documentation-authorizedIp")))
