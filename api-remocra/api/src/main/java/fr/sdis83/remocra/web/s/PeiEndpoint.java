...
public class PeiEndpoint {
  ...
  @Operation(
      summary =
          "Retourne la liste des PEI en lien avec l'organisme associé à la clef d'accès d’API. Le lien est "
  ...
  public Response getPei(
      final @Parameter(
              description =
                  "Nombre maximum de résultats à retourner (maximum fixé à 200 résultats)")
      ...
      final @Parameter(description = "Numéro INSEE de la commune où se trouve le PEI") @QueryParam(
              "insee") String insee,
      ...
      final @Parameter(description = "Nature DECI de l'hydrant") @QueryParam("natureDECI") String
              natureDECI) {
    return Response.ok(this.peiUseCase.getAll(insee, type, nature, natureDECI, limit, start))
        .build();
  }
  ...
  @Path("/{numero}")
  @Operation(
      summary = "Retourne les informations communes à tout type de PEI d'un PEI spécifique",
      tags = {"DECI - Points d'Eau Incendie"})
  @RolesAllowed({UserRoles.RoleTypes.RECEVOIR})
  ...
  public Response getPeiCaracteristiques(
      ...
      throws JsonProcessingException {
    try {
      return Response.ok(peiUseCase.getPeiCaracteristiques(numero), MediaType.APPLICATION_JSON)
          .build();
    } catch (ResponseException e) {
      return Response.status(e.getStatusCode()).entity(e.getMessage()).build();
    }
  ...
  @Operation(
      summary =
          "Modifie les caractéristiques techniques propres au PEI et à son type (PIBI ou PENA)",
      tags = {"DECI - Points d'Eau Incendie"})
  ...
  public Response updatePeiCaracteristiques(
    ...
    try {
      return Response.ok(
              peiUseCase.updatePeiCaracteristiques(numero, peiForm), MediaType.APPLICATION_JSON)
          .build();
    } catch (ResponseException e) {
      return Response.status(e.getStatusCode()).entity(e.getMessage()).build();
    }
  ...
  @Operation(
      summary =
          "Liste des PEI ayant subit une modification (ajout, modification ou suppression) postérieure au *moment* passée en paramètre",
      tags = {"DECI - Points d'Eau Incendie"})
  @RolesAllowed({UserRoles.RoleTypes.RECEVOIR})
