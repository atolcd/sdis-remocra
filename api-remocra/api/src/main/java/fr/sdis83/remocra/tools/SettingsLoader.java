...
public static Config load(Path path) {
        ...
        if (path != null) {
                if (Files.isRegularFile(path) && Files.isReadable(path)) {
                        applicationConfig = ConfigFactory.parseFileAnySyntax(path.toFile()).withFallback(
                                ConfigFactory.parseMap(Collections.singletonMap("application.conf-dir", path.getParent()
        ...
        return constConfig.withFallback(ConfigFactory.defaultOverrides()).withFallback(applicationConfig)
