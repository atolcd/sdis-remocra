...
public class HydrantVisitesRepository {
  ...
  public List<HydrantVisiteModel> getAll(String numero, String contexte, ZonedDateTime moment, Boolean derniereOnly, Integer start, Integer limit) throws IOException {
  ...
  public Integer getNbVisites(Long idHydrant, ZonedDateTime momentVisite) {
    return context
            .selectCount()
            .from(HYDRANT_VISITE)
            .where(HYDRANT_VISITE.HYDRANT.eq(idHydrant).and(HYDRANT_VISITE.DATE.eq(momentVisite.toInstant())))
