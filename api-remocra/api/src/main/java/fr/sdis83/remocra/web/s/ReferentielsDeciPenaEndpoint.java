...
public class ReferentielsDeciPenaEndpoint {
  ...
  @Inject TypeHydrantMateriauRepository typeHydrantMateriauRepository;

  @Inject TypeHydrantAspirationRepository typeHydrantAspirationRepository;
  ...
  @Path("/naturesPEI")
  @Operation(
      summary = "Retourne les natures de PEI possibles pour les PEI de type PENA",
  ...
  @Operation(
      summary = "Retourne les matériaux possibles pour les PEI de type PENA",
      tags = {"DECI - Référentiels PENA"})
  @RolesAllowed({UserRoles.RoleTypes.RECEVOIR})
  ...
  @Operation(
      summary =
          "Retourne les types d'anomalies pouvant être constatées pour une nature de PENA (ex : Envasement "
  ...
  public Response getRefentielNaturesAnomalies(
    ...
    return Response.ok(
            typeHydrantNatureAnomalieRepository.getAll(
                codeNature, contexteVisite, "PENA", start, limit))
        .build();
  }
}
