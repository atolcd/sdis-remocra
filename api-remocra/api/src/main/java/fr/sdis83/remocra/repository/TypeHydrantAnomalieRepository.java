...
public class TypeHydrantAnomalieRepository {
  ...
  public int getNbAnomaliesChecked(
      String codeTypeVisite, String codeTypeHydrantNature, List<String> controlees) {
    return context
        .selectCount()
        ...
        .on(TYPE_HYDRANT_ANOMALIE_NATURE.NATURE.eq(TYPE_HYDRANT_NATURE.ID))
        .where(
            TYPE_HYDRANT_SAISIE
                .CODE
                .equalIgnoreCase(codeTypeVisite)
  ...
  public void deleteAnomaliesNonSysteme(Long peiId) {
    Collection<Long> anomaliesSysteme =
        getAnomaliesSysteme().stream().map(TypeHydrantAnomalie::getId).collect(Collectors.toList());
    ...
    context
        ...
        .where(
            HYDRANT_ANOMALIES
                .HYDRANT
                .eq(peiId)
                .and(HYDRANT_ANOMALIES.ANOMALIES.notIn(anomaliesSysteme)))
        .execute();
  }
