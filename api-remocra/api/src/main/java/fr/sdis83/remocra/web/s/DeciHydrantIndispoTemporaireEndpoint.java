...
public class DeciHydrantIndispoTemporaireEndpoint {
  ...
  @Inject IndispoTemporaireUseCase indispoTemporaireUseCase;
  ...
  public Response getIndispoTemporaire(
      final @Parameter(
              description =
                  "Code de l'organisme API étant à l'origine de l'indisponibilité temporaire")
          @QueryParam("organismeApi") String organismeAPI,
      final @Parameter(
              description = "Ne renvoie que les indisponibilités temporaires liées à cet hydrant")
          @QueryParam("numeroHydrant") String hydrant,
      final @Parameter(description = "Code du statut de l'indisponibilité temporaire") @QueryParam(
      ...
      throws IOException {
    return Response.ok(
            this.indispoTemporaireUseCase.getAll(organismeAPI, hydrant, statut, limit, start))
        .build();
  }
  ...
  @Path("")
  @Operation(
      summary = "Ajoute une nouvelle indisponibilité temporaire",
      tags = {"DECI - Indispo temporaire"})
  ...
  public Response addIndispoTemporaire(
      @NotNull @Parameter(description = "Informations de l'indisponibilité temporaire")
          IndispoTemporaireForm indispoForm) {
    try {
      return Response.ok(
              indispoTemporaireUseCase.addIndispoTemporaire(indispoForm),
  ...
  @ApiResponse(
      responseCode = "200",
      description = "Indisponibilité temporaire modifiée avec succès")
  @ApiResponse(responseCode = "400", description = "Erreur à la saisie")
  @RolesAllowed({UserRoles.RoleTypes.TRANSMETTRE})
  public Response editIndispoTemporaire(
      final @Parameter(description = "Identifiant de l'indisponibilité temporaire)") @PathParam(
              "idIndisponibiliteTemporaire") String idIndispo,
      @Parameter(description = "Informations d'indisponibilite temporaire", required = true)
          @NotNull
          IndispoTemporaireSpecifiqueForm form)
