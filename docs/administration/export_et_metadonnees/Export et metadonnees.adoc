...
|===
.Exemple de fichier XML de définition d'un export au format CSV (export non spatial) de toutes les communes qui intersectent une zone de compétence spécifique
[source,xml]
include::src/commune_shp.xml[]

.Exemple de fichier XML de définition d'un export au format Esri Shapefile (export spatial) de toutes les communes qui intersectent une zone de compétence spécifique
[source,xml]
include::src/commune_csv.xml[]

=== Référencement des modèles d'export

Les fichiers XML de modèles doivent être déposés par l'administrateur de la plate-forme via FTP (utilisateur FTP _"remocra"_) dans le sous-dossier _"export"_ (dossier à créer s'il n'existe pas) présent dans dossier REMOcRA dédié aux modèles (le chemin défini dans les paramètres de configuration de l'application avec le libellé _"Dossier de stockage des modèles de documents utilisés par l'ETL Pentaho Data Integration"_. La valeur par défaut est _"/var/remocra/modeles"_.
