...
begin
    ...
    numero_patch := 138;
    description_patch :='Recherche PEI le plus proche';
...
('DECI_DISTANCE_MAX_PARCOURS', 'Distance maximum de recherche sur le réseau routier', '300', 1, 'Planification DECI');

CREATE OR REPLACE FUNCTION couverture_hydraulique.plus_proche_pei(
  geomclic geometry,
  distance_max_parcours integer,
  idreseauimporte integer)
    RETURNS TABLE(pei integer, chemin text, dist float)
    LANGUAGE 'plpgsql'
...
BEGIN
  ...
  PERFORM couverture_hydraulique.inserer_jonction_pei(p.id, distance_max_au_reseau, idReseauImporte)
