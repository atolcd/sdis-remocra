...
UPDATE remocra.param_conf
SET valeur = 'WMS'
WHERE cle = 'HYDRANT_SYMBOLOGIE_METHODE';
...
UPDATE remocra.param_conf
SET valeur = 'numero%natureNom%nomNatureDeci%adresse%nomCommune%dispoTerrestre%dateContr%dateReco%nomTournee'
...
FROM
	(SELECT xmlelement(name "hydrant",
			...
			xmlelement(name "diametre", h.diametre)
	) as hydrant_data
	FROM 
	...
	FROM
		...
		as a) as anomalies,
	(
		SELECT XMLELEMENT(name "observations", hv.observations) AS observations
		FROM 
			(
				SELECT DISTINCT ON (hv.hydrant) hv.observations 
				FROM remocra.hydrant_visite hv
				WHERE hv.hydrant = ${HYDRANT_ID}
...
-- Anomalies non conforme
UPDATE remocra.type_hydrant_anomalie
SET code = code||'_NC'
WHERE code IN ('ACCES_DIF', 'MUR_OBST', 'ABS_AIRE', 'OUV_DIF', 'VIDANGE', 'FUITE', 'NON_NORM');

-- Trigger debit pression
...
CREATE OR REPLACE FUNCTION remocra.trg_calcul_debit_pression_21()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
	...
	perform remocra.calcul_debit_pression_21(p_rec.id);
...
CREATE TRIGGER trig_debit_pression_21 AFTER
INSERT
    OR
UPDATE
    ON
    remocra.hydrant_pibi FOR EACH ROW EXECUTE PROCEDURE remocra.trg_calcul_debit_pression_21();
