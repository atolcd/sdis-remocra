...
--------------------------
CREATE OR REPLACE
FUNCTION remocra.calcul_debit_pression_39(id_hydrant bigint) RETURNS VOID LANGUAGE plpgsql AS $FUNCTION$
DECLARE
...
BEGIN IF
(TG_OP = 'DELETE') THEN p_rec = OLD;
ELSE p_rec = NEW;
END IF;

PERFORM
remocra.calcul_debit_pression_39(p_rec.id);
...
PERFORM
remocra.calcul_volume_39(p_rec.id);
