...
BEGIN
        ...
        (
            ...
            where code
            IN(
                'PRESSION_INSUFF', 'PRESSION_TROP_ELEVEE', 'PRESSION_DYN_INSUFF',
        ...
        if ((p_rec.debit < 30) AND p_rec.diametre = p_diametre_id) then
        ...
        elsif (p_rec.debit > 90  AND p_rec.diametre = p_diametre_id) then
        ...
        if ((p_rec.debit < 30)  AND p_rec.diametre = p_diametre_id) then
            select id into p_anomalie_id from remocra.type_hydrant_anomalie where code = 'DEBIT_INSUFF';
            insert into remocra.hydrant_anomalies (hydrant,anomalies) values (p_rec.id, p_anomalie_id);
        elsif (p_rec.debit < 60  AND p_rec.diametre = p_diametre_id) then
            select id into p_anomalie_id from remocra.type_hydrant_anomalie where code = 'DEBIT_INSUFF_NC';
            insert into remocra.hydrant_anomalies (hydrant,anomalies) values (p_rec.id, p_anomalie_id);
        elsif (p_rec.debit > 130  AND p_rec.diametre = p_diametre_id) then
            select id into p_anomalie_id from remocra.type_hydrant_anomalie where code = 'DEBIT_TROP_ELEVE';
            insert into remocra.hydrant_anomalies (hydrant,anomalies) values (p_rec.id, p_anomalie_id);
        end if;
        ...
        if ((p_rec.debit < 30)  AND p_rec.diametre = p_diametre_id) then
            select id into p_anomalie_id from remocra.type_hydrant_anomalie where code = 'DEBIT_INSUFF';
            insert into remocra.hydrant_anomalies (hydrant,anomalies) values (p_rec.id, p_anomalie_id);
        elsif (p_rec.debit < 120  AND p_rec.diametre = p_diametre_id) then
            select id into p_anomalie_id from remocra.type_hydrant_anomalie where code = 'DEBIT_INSUFF_NC';
            insert into remocra.hydrant_anomalies (hydrant,anomalies) values (p_rec.id, p_anomalie_id);
        elsif (p_rec.debit > 180  AND p_rec.diametre = p_diametre_id) then
            select id into p_anomalie_id from remocra.type_hydrant_anomalie where code = 'DEBIT_TROP_ELEVE';
            insert into remocra.hydrant_anomalies (hydrant,anomalies) values (p_rec.id, p_anomalie_id);
        end if;
        ...
        if (p_rec.pression_dyn < 1) then
            ...
            insert into remocra.hydrant_anomalies (hydrant,anomalies) values (p_rec.id, p_anomalie_id);
        elsif (p_rec.pression_dyn > 8) then
