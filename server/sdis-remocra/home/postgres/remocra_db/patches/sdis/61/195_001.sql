...
AS WITH anomalie AS (
 ...
 SELECT h.numero AS num,
   ...
   FROM hydrant h
     ...
     JOIN type_hydrant_nature_deci thnd ON h.nature_deci = thnd.id
     ...
     LEFT JOIN hydrant_tournees ht ON h.id = ht.hydrant
     LEFT JOIN tournee t ON ht.tournees = t.id
     LEFT JOIN organisme o_tournee ON t.affectation = o_tournee.id;