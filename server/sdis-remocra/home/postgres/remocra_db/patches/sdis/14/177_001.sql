BEGIN;
CREATE OR REPLACE FUNCTION remocra.calcul_debit_pression_14(id_hydrant bigint)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
...
BEGIN
	...
	if (p_rec.pression_dyn < 1) then
		...
		insert into remocra.hydrant_anomalies (hydrant,anomalies) values (p_rec.id, p_anomalie_id);
    elsif (p_rec.pression_dyn >= 6) then
...
CREATE OR REPLACE FUNCTION remocra.trg_calcul_debit_pression_14()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
	...
	perform remocra.calcul_debit_pression_14(p_rec.id);
    RETURN p_rec;
END;
$function$
;
ALTER TABLE remocra.hydrant_pibi DISABLE TRIGGER trig_debit_pression;
DROP TRIGGER IF EXISTS trig_debit_pression_14 ON remocra.hydrant_pibi;
CREATE TRIGGER trig_debit_pression_14 AFTER
INSERT
    OR
UPDATE
    ON
    remocra.hydrant_pibi FOR EACH ROW EXECUTE PROCEDURE remocra.trg_calcul_debit_pression_14();
