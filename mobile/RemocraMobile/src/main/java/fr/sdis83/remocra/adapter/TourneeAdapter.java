...
public class TourneeAdapter extends CursorAdapter {
    ...
    private static final String[] projection = new String[]{
            ...
            "(select (100 * (  " +
                    "sum(" + HydrantTable.COLUMN_STATE_H1 + ") + " +
                    "sum(" + HydrantTable.COLUMN_STATE_H2 + ") + " +
                    "sum(" + HydrantTable.COLUMN_STATE_H3 + ") + " +
                    "sum(" + HydrantTable.COLUMN_STATE_H4 + ") + " +
                    "sum(" + HydrantTable.COLUMN_STATE_H5 + ") + " +
                    "sum(" + HydrantTable.COLUMN_STATE_H6 + "))) " +
                    " / ( 6 * " + TourneeTable.TABLE_NAME + "." + TourneeTable.COLUMN_NB_HYDRANT + ")" +
                    " from " + HydrantTable.TABLE_NAME +
                    " where " + HydrantTable.COLUMN_TOURNEE + " = " + TourneeTable.TABLE_NAME + "." + TourneeTable._ID + ") " + TourneeTable.FIELD_TOTAL_HYDRANT
    };
