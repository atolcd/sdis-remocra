		...
		<div v-else>
			<div v-if="errorOnDataLoad">
				<b-alert variant="danger" show>
					<p>Une erreur est survenue lors du chargement des données de ce point d'eau <br />
					Le résumé ne peut être affiché </p>
	...
	methods: {
		...
		loadData: function(){
			axios.get("/remocra/requetefiche/resume/"+this.hydrantRecord.id+"?useDefault="+!this.useCustomResume).then(response => {
				parseString(response.data.toString(), { explicitArray: false}, (err, result) => {
